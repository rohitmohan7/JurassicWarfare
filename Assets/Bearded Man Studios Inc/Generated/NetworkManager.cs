...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] PlayerCreatureNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is PlayerCreatureNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerCreatureNetworkObject.Length > 0 && PlayerCreatureNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlayerCreatureNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlayerCreatureBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiatePlayerCreature instead, its shorter and easier to type out ;)")]
		public PlayerCreatureBehavior InstantiatePlayerCreatureNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerCreatureNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerCreatureBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PlayerCreatureBehavior>().networkObject = (PlayerCreatureNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of PlayerCreature
		/// </summary>
		/// <returns>
		/// A local instance of PlayerCreatureBehavior
		/// </returns>
		/// <param name="index">The index of the PlayerCreature prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlayerCreatureBehavior InstantiatePlayerCreature(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerCreatureNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerCreatureBehavior>();
